...
function getLocalBest(){
   return (localStorage.getItem('bestScore') !== null)?localStorage.getItem('bestScore'):-1;
}
function setLocalBest(num1,num2){
    var localBest = Math.max(num1,num2);
    localStorage.setItem('bestScore',localBest);
    ...
    for( var i = 0 ; i < 5 ; i ++ )
        for( var j = 0 ; j < 5 ; j ++ ){
            ...
            if( board[i][j] == 0 ){
                ...
                theNumberCell.css('font-size',getFontSize(2));//theNumberCell.css('font-size',0.6*cellSideLength+'px');//0.6*cellSideLength+
            }
            else{
